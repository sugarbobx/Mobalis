-- MOBALIS — correctif migration 0013
-- get_bulletin_sequence() était déclarée `stable`, mais Postgres interdit
-- CREATE TEMPORARY TABLE dans une fonction non-volatile ("CREATE TABLE AS
-- is not allowed in a non-volatile function") — confirmé par test e2e réel.
-- Même corps, sans `stable` (volatile par défaut).
