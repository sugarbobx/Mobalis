-- MOBALIS — correctif evaluer_badges (migration 0021)
-- "Champion de la semaine" s'attribuait trivialement dès qu'un élève était
-- le SEUL participant soumis à un défi hebdo (il est alors "meilleur score"
-- par défaut, cohorte de 1). Ajoute une exigence d'au moins 2 participants
-- notés sur ce défi — sinon "champion" n'a pas de sens.

create or replace function evaluer_badges(p_eleve_id uuid)
returns jsonb
...
begin
  ...
  select count(*) = 5 and count(*) filter (where present) = 5
  into v_flag
  from (
  ...
  if v_flag then
    v_codes_acquis := array_append(v_codes_acquis, 'assidu');
  end if;
  ...
  if v_flag then
    v_codes_acquis := array_append(v_codes_acquis, 'sans_faute');
  end if;

  -- Régulier : aucun devoir en retard sur 30 jours, ≥3 rendus.
  select v.pas_de_retard and v.nb_rendus >= 3
  into v_flag
  from (
    with devoirs_recents as (
  ...
  if v_flag then
    v_codes_acquis := array_append(v_codes_acquis, 'regulier');
  end if;

  -- Progression : +3 pts de moyenne (mois courant vs mois précédent).
  select coalesce(v.moyenne_recente - v.moyenne_precedente >= 3, false)
  into v_flag
  from (
    with m as (
    ...
    select
      (select moyenne from m where rn = 1) as moyenne_recente,
      (select moyenne from m where rn = 2) as moyenne_precedente
  ) v;
  if v_flag then
    v_codes_acquis := array_append(v_codes_acquis, 'progression');
  end if;
  ...
  if v_flag then
    v_codes_acquis := array_append(v_codes_acquis, 'premier_defi');
  end if;
  ...
  if v_flag then
    v_codes_acquis := array_append(v_codes_acquis, 'serie_defis');
  end if;

  -- Champion de la semaine : meilleur score sur son dernier défi hebdo soumis,
  -- parmi au moins 2 participants notés (une cohorte de 1 n'a pas de sens).
  select coalesce(
    dd.score is not null and dd.nb_participants >= 2 and dd.score >= all (
  ...
  from (
    select dp.defi_id, dp.score,
           (select count(*) from defi_participations where defi_id = dp.defi_id and score is not null) as nb_participants
  ...
  if v_flag then
    v_codes_acquis := array_append(v_codes_acquis, 'champion_semaine');
  end if;
  ...
  if v_flag then
    v_codes_acquis := array_append(v_codes_acquis, 'maitre_chapitre');
  end if;
  ...
  if v_flag then
    v_codes_acquis := array_append(v_codes_acquis, 'mention_excellence');
  end if;
