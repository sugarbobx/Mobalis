-- MOBALIS — correctif evaluer_badges (migration 0019)
-- Bug réel trouvé par e2e : 3 des 9 critères (Assidu, Régulier, Progression)
-- référençaient un alias de sous-requête FROM comme s'il s'agissait d'une
-- CTE réutilisable ("select ... from alias" dans une sous-requête imbriquée
-- de la même liste SELECT) — invalide, "relation ... does not exist" à
-- l'exécution. Corrigé : Assidu agrège directement sur la sous-requête ;
-- Régulier/Progression utilisent une vraie CTE (with ... ) enveloppée dans
-- une sous-requête FROM, réutilisable dans toute la liste SELECT.
